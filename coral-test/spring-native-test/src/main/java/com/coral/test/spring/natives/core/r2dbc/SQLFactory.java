package com.coral.test.spring.natives.core.r2dbc;

import com.coral.test.spring.natives.config.R2dbcConfig;
import com.coral.test.spring.natives.core.enums.GlobalOrder;
import com.coral.test.spring.natives.core.utils.NameStyleUtil;
import com.coral.test.spring.natives.core.utils.StrFormatter;
import com.coral.test.spring.natives.core.utils.reflection.IGetter;
import com.coral.test.spring.natives.core.utils.reflection.LambdaFieldUtil;
import com.coral.test.spring.natives.dto.PageInfo;
import com.coral.test.spring.natives.dto.req.PageQueryDTO;
...
public class SQLFactory {
     ...
     * @return
     */
    public static <T, R> Mono<PageInfo<T>> pageQuery(R2dbcEntityTemplate template, Query query, Class<R> entityClass, PageInfo<T> pageInfo) {
        return template.count(query, entityClass)
                .flatMap(total -> template.select(query, entityClass)
                        .collectList()
                        .map(list -> pageInfo.total(Math.toIntExact(total)).records((List<T>) list))
                );
    }
